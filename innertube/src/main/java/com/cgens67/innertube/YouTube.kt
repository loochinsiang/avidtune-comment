...
object YouTube {
    ...
    suspend fun searchSummary(query: String): Result<SearchSummaryPage> = runCatching {
        ...
        val contents = response.contents?.tabbedSearchResultsRenderer?.tabs?.firstOrNull()?.tabRenderer?.content?.sectionListRenderer?.contents ?: emptyList()
    ...
    suspend fun search(query: String, filter: SearchFilter): Result<SearchResult> = runCatching {
        ...
        val sectionListContents = response.contents?.tabbedSearchResultsRenderer?.tabs?.firstOrNull()
            ?.tabRenderer?.content?.sectionListRenderer?.contents

        val musicShelfRenderer = sectionListContents?.lastOrNull()?.musicShelfRenderer
            ?: sectionListContents?.lastOrNull()?.itemSectionRenderer?.contents?.lastOrNull()?.musicShelfRenderer

        SearchResult(
            items = musicShelfRenderer?.contents?.getItems()?.mapNotNull {
                SearchPage.toYTItem(it)
            }.orEmpty(),
            continuation = musicShelfRenderer?.continuations?.getContinuation() ?: musicShelfRenderer?.contents?.getContinuation()
        )
    }
    ...
    suspend fun searchContinuation(continuation: String): Result<SearchResult> = runCatching {
        ...
        response.continuationContents?.musicShelfContinuation?.let { shelf ->
            items.addAll(shelf.contents?.mapNotNull { it.musicResponsiveListItemRenderer?.let(SearchPage::toYTItem) }.orEmpty())
            nextContinuation = shelf.continuations?.getContinuation()
        }

        response.continuationContents?.sectionListContinuation?.let { sectionList ->
            sectionList.contents?.forEach { sectionContent ->
                sectionContent.musicShelfRenderer?.let { shelf ->
                    items.addAll(shelf.contents?.getItems()?.mapNotNull { SearchPage.toYTItem(it) }.orEmpty())
                }
                sectionContent.itemSectionRenderer?.contents?.forEach { itemContent ->
                    itemContent.musicResponsiveListItemRenderer?.let { renderer ->
                        SearchPage.toYTItem(renderer)?.let { items.add(it) }
                    }
                }
            }
        ...
        response.continuationContents?.itemSectionContinuation?.let { itemSection ->
            itemSection.contents?.forEach { itemContent ->
                itemContent.musicResponsiveListItemRenderer?.let { renderer ->
                    SearchPage.toYTItem(renderer)?.let { items.add(it) }
                }
            }
            nextContinuation = itemSection.continuations?.getContinuation()
        }
        
        response.onResponseReceivedActions?.firstOrNull()?.appendContinuationItemsAction?.continuationItems?.let { continuationItems ->
            items.addAll(continuationItems.mapNotNull { it.musicResponsiveListItemRenderer?.let(SearchPage::toYTItem) })
            if (nextContinuation == null) {
                nextContinuation = continuationItems.getContinuation()
            }
        }
    ...
    suspend fun albumSongs(playlistId: String): Result<List<SongItem>> =
        runCatching {
            ...
            val content = response.contents?.twoColumnBrowseResultsRenderer
                ?.secondaryContents
                ?.sectionListRenderer
                ?.contents
                ?.firstOrNull()

            val shelfContents = content?.musicPlaylistShelfRenderer?.contents
            ...
            while (continuation != null) {
                ...
                val continuationContents = response.continuationContents?.musicPlaylistShelfContinuation?.contents
                    ?: response.continuationContents?.musicShelfContinuation?.contents

                songs += continuationContents?.getItems()?.mapNotNull {
                    AlbumPage.fromMusicResponsiveListItemRenderer(it)
                }.orEmpty()
